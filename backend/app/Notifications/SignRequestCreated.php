...
class SignRequestCreated extends Notification
{
    use Queueable;
    ...
    {
        // Dùng submitter_name accessor — tự xử cá nhân/nhóm
        $submitterName = $this->signRequest->submission->submitter_name ?? 'Không xác định';

        return [
            'title'   => 'Yêu cầu ký số mới',
            'content' => "{$submitterName} vừa gửi yêu cầu ký số tài liệu cần bạn xử lý.",
            'type'    => 'sign_request',
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $createdAt     = $this->signRequest->created_at?->format('d/m/Y H:i') ?? now()->format('d/m/Y H:i');

        return (new MailMessage)
            ->subject("[Ký số] Yêu cầu ký tài liệu mới - #{$this->signRequest->id}")
            ->greeting("Kính chào {$notifiable->name},")
            ->line("Sinh viên đã gửi yêu cầu ký số tài liệu trong lớp bạn phụ trách.")
            ->line("- Loại tài liệu: {$documentLabel}")
            ->line("- Nhóm/SV: {$submitterName}")
            ->line("- Lớp: {$className}")
            ->line("- Người gửi: {$studentName}")
            ->line("- Thời gian gửi: {$createdAt}")
            ->action(
                'Xem và xử lý',
                config('app.frontend_url') . "/lecturer/sign-requests/{$this->signRequest->id}"
            )
            ->line('Lưu ý: Bạn cần đăng ký chữ ký số trước khi thực hiện ký tài liệu.')
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            ...
            'created_at'      => $this->signRequest->created_at?->toISOString(),
            'message'         => "Bạn có yêu cầu ký số {$documentLabel} mới từ {$submitterName}.",
        ];
    }
}